...
def loadStress(femPath):
    with open(femPath) as f:
        ...
        while True:
            ...
            stressList.append(
                [
                    [float(strlist[2]), float(strlist[5]), float(strlist[6])],
...
def getWeightsMatrix(tet_cage, node):
    ...
    tet_cage_elem_max_length = (
        tet_cage_vertices_repeats - tet_cage_center_repeat4
    ).reshape((-1, 3))
    rSize = (
        np.linalg.norm(tet_cage_elem_max_length, axis=1).max() * 1.5
    )  # actually sqrt(3) is enough
    ...
    with open(filepath, "w") as file:
        ...
        for it in f:
            if vn is None and vt is None:
                ...
                file.write(
                    "f {}/{}/{} {}/{}/{} {}/{}/{}\n".format(
                        it[0], it[0], it[0], it[1], it[1], it[1], it[2], it[2], it[2]
                    )
                )
